...
bool treatMessage()
{
    ...
    else if (IS_MSG(SET_THRUST_MSGID))
    {
        ...
        thresholdRange(&lValue, 0.0f, 20.0f);
        setMeanForce(lValue);
    ...
    else if (IS_MSG(SET_PID_MSGID))
    {
        ...
        // Set de la valeur
        setPIDCoeffs( FLOAT(0*sizeof(float)),
                      FLOAT(1*sizeof(float)),
                      FLOAT(2*sizeof(float)) );

        // Get de la valeur pour retour
        float P, I, D;
        getPIDCoeffs(&P, &I, &D);
        
        formatText("P: %f, I: %f, D: %f\n", P, I, D);
    }
    ...
    {
        CHECK_SIZE(sizeof(float));
        setServoAngle(FLOAT(0));
        
        float lValue = getServoAngle();
        formatText("Servo: %f\n", lValue);
    }
